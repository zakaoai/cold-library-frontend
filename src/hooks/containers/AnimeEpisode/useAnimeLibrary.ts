...
export default function useAnimeLibrary(malId: number) {
  ...
  const { data, isFetched, isFetching } = useQueries({
    queries: [
      { queryKey: ["api.anime.get", malId], queryFn: () => AnimeServices.get(malId) },
      { queryKey: ["api.trackedAnimeTorrent.get", malId], queryFn: () => TrackedAnimeTorrentService.get(malId) }
    ],
    combine
  });
  ...
  const updateAnime = (updatedAnime: AnimeDTO) => setAnime(anime => ({ ...anime, ...updatedAnime }));
